...
public class Kadai04_3 extends Application
{
	...
	private void drawClock(GraphicsContext gc, int x, int y, int r)
	{
		gc.setLineWidth(1);
		...
		for (int t = 0; t < 60; t++)
		{
			int len = (int)((t % 5 == 0) ? RADIUS / 10.0 : RADIUS / 20.0);
			double rad = t / 30.0 * Math.PI;
			int tmp_x = (int)(x + (r - len)*Math.cos(rad));
			int tmp_y = (int)(y + (r - len)*Math.sin(rad));
			gc.strokeLine(tmp_x, tmp_y, tmp_x + (int)(len * Math.cos(rad)), tmp_y + (int)(len * Math.sin(rad)));
		}
	}
}